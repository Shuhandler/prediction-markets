...
Everything lives in a single file, `arb_bot.py` (~7,500 lines, ~37 classes). README.md documents each component in depth — consult it before changing behavior. `scope.md` is the phase roadmap (Phases 0–6 done; remaining: 7 economics gate, 8 auto-discovery; plus a Code-Review Follow-ups section with open design decisions). `IBKR_ROLLOUT.md` plans the parallel ForecastEx/IBKR venue-expansion track (phases FX-A…FX-D; not started).
...
- **Polymarket FOK is always Leg 1.** If it fails, no Kalshi order is placed — zero legging risk. Kalshi IOC (Leg 2) supports partial fills; excess Leg 1 contracts get unwound. Leg 2 goes out **immediately** after Leg 1 fills (no book refetch — latency is legging risk) at a **reservation-price ceiling**: the highest Kalshi price still netting `MIN_PROFIT_MARGIN` after fees (`kalshi_reservation_price()`). The CLOB fills at book prices, so the common case executes at the observed ask while a moved book fills at any still-acceptable deeper level instead of forcing an unwind. The executed VWAP (`average_fill_price`, side-normalized in `KalshiOrderClient.place_order`) is booked onto the leg and realized net recomputed; fill paths without a VWAP book the wire ceiling (conservative). `ExecutionEngine` sizes/reserves capital at the ceiling price, since fills can legitimately land above the observed ask.
